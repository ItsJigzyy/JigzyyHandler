module.exports = {
    ...
    execute(client, message, args) {
        ...
        const id = args[0]
        if (!args[0]) return message.channel.send("Please provide a `Steam64 ID`!");
        if (isNaN(args[0][0])) return message.channel.send("Steam ID 64 only please!")
        ...
        fetch(summaries).then(results => results.json()).then(body => {
            ...
            if (body.response.success === 42) return message.channel.send('Unable to find a steam profile with that ID\nMake sure its the Steam ID 64!');
